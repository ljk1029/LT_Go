...
//文字检测
type IAliYunClientWords interface {
	GetResponseWord(path string, clinetInfo ClinetInfo, bizData BizDataWords) string
}
func (defaultClient DefaultClient) GetResponseWord(path string, clinetInfo ClinetInfo, bizData BizDataWords) string{
	clientInfoJson, _ := json.Marshal(clinetInfo)
	bizDataJson, _ := json.Marshal(bizData)
