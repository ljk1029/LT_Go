...
var (
	...
	Table = "merge_video_template"
	// Model is the model object of user.
	Model, Columns = &arModel{g.DB("default").Table(Table).Safe()}, struct {
		Id               string // 模板id
		Title            string // 模板标题
		Content          string // 模板内容
		VideoTemplateUrl string // 模板视频地址
		Thumbnail        string // 模板缩略图
		FixedPicNum      string // 固定图片数
		TemplateFunc     string // 固定图片数
		IsHead           string // 固定图片数
		IsShow           string // 固定图片数
		DefaultTitle     string // 固定图片数
	}{
		Id:               "id",
		Title:            "title",
		Content:          "content",
		VideoTemplateUrl: "video_template_url",
		Thumbnail:        "thumbnail",
		FixedPicNum:      "fixed_pic_num",
		TemplateFunc:     "template_func",
		IsHead:           "is_head",
		IsShow:           "is_show",
